...
$(document).ready(() => {
    const links = [{
        ...
        {
            name: 'GarxuShop',
            link: 'https://shop.garxu.xyz',
        },
        {
            name: 'LightningBot',
            link: 'https://lightning-bot.com',
        },
        {
            name: 'Steam Profile',
            link: '#',
    ...
    for (let i in links) {
        ...
        if (i != links.length - 1) $('#marquee').append(' <img class="emoticon" src="assets/others/cheese.png"> ');
    }
    ...
    app.titleChanger(['Thighs Connoisseur', 'Cheddlatron Dev', 'Official Cheddar']);
});
...
$('html').on('contextmenu', (event) => {
    ...
    img.src ='https://garxu.xyz/top/assets/others/trollface.jpg';
    img.width = 64;
    img.height = 64;
    img.alt = 'ched.rip';
...
$.getJSON(ipgeolocation, (data) => {
    writeLine(['https://garxu.xyz/auth/', "Granting access to <span style='font-size: 14px; color: #e87aff;'>[Garxu.xyz]</span>!"], 30, () => {
        ...
        writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `Welcome back, <i style='color: #0f0'>${ip}</i>! By the way, nice to see someone from ${country} here!`], 30, 500, () => {
            ...
            writeLine([`<i style='color: #F62459'>garxu.xyz ☭♛⛧</i>`], 120, 500, () => {
                timeouts.push(
                    setTimeout(() => {
                        if (app.skippedIntro) return;
